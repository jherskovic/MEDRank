...
# encoding: utf-8
"""
test_mm_prox_cooc_graph_builder.py
...
import unittest
import sys
from MEDRank.computation.metamap_proximity_cooccurrence_graph_builder import *
sys.path.append('../') # Necessary for testing
sys.path.append('../../')
from MEDRank.computation.graph import Graph
from MEDRank.utility.logger import *

# TODO: Create tests
class test_mm_prox_cooc_graph_builder(unittest.TestCase):
    def setUp(self):
    ...
    def testMetamapBuilderEmptySet(self):
        my_builder=MetamapProximityCooccurrenceGraphBuilder()
        graph=my_builder.create_graph([])
        self.assert_(type(graph) is Graph)
    def testMetamapBuilderBuildsSingleGraphCorrectly(self):
        my_builder=MetamapProximityCooccurrenceGraphBuilder()
        lines=[x.lines for x in self.mo]
        self.assertEqual(1, len(lines)) # Only one set of lines
        lines=lines[0]
        graph=my_builder.create_graph(lines)
